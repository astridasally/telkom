...
class DataExcel implements ToModel, WithHeadingRow, WithMapping
{
    ...
    public function map($row): array
    {
        return [
            ...
            'ihld'                 => $row['ihld'],
            'plan_survey'          => $this->toDate($row['plan_lainnya']),
            'realisasi_survey'     => $this->toDate($row['realisasi_lainnya']),
            'plan_delivery'        => $this->toDate($row['plan_mos']),
            'realisasi_delivery'   => $this->toDate($row['realisasi_mos']),
